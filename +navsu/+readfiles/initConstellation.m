...
constEnums = [navsu.ConstEnum.GPS navsu.ConstEnum.GLO navsu.ConstEnum.GAL...
    navsu.ConstEnum.BDS navsu.ConstEnum.QZSS navsu.ConstEnum.SBAS];
...
for i = 1 : numel(systems)
    if(constellations.(systems{i}).enabled)
        ...
        %         constInd = constEnums(find(~cellfun(@isempty,strfind(constMap,systems{i}))));
        %         constellations.constInds = [constellations.constInds repelem(constInd,size(indexes_tmp,1),size(indexes_tmp,2))];
    end
end
