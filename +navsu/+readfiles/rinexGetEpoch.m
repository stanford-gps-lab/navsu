function [time, datee, num_sat, sat, sat_types, tow] = rinexGetEpoch(fid)

% SYNTAX:
%   [time, datee, num_sat, sat, sat_types, tow] = rinexGetEpoch(fid);
%
% INPUT:
...
%             year = four_digit_year(year);
            ...
            [week,sowDay] = navsu.time.jd2gps(navsu.time.cal2jd(year,month,day));
        ...
        if (strcmp(lin(32),'0') || strcmp(lin(32),'1') || strcmp(lin(32),'2'))
            ...
            %computation of the GPS time in weeks and seconds of week
            time = navsu.time.cal2epochs([year, month, day, hour, minute, second]);
            [~,tow] = navsu.time.epochs2gps(time);
