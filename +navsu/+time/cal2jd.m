function jds=cal2jd(yrs,mns,dys)
...
nTimes = max([length(dys) length(mns) length(yrs)]);


if length(dys)+length(mns)+length(yrs) ~=3 && nTimes + 2 ~= length(dys)+length(mns)+length(yrs) && ~(length(dys) == length(yrs) && length(dys) == length(mns))
    warning('Invalid vector lengths')
end

if length(dys) == 1
   dys = repmat(dys,nTimes,1); 
end

if length(mns) == 1
   mns = repmat(mns,nTimes,1); 
end

if length(yrs) == 1
   yrs = repmat(yrs,nTimes,1); 
end


jds = nan(size(yrs));

for idx = 1:length(yrs)
