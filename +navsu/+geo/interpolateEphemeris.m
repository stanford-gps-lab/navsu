...
for idx = idx_start:idx_end
    ...
    % Pxyzti(1 + rcjdx, 1) output should be a 9x1 (for 5 min -> 30 seconds
    % rate)
    ...
    % add interpolated clock bias
    Pxyzti(1 + [0 rcjdx], 4) = Cclock(cdx);

    % add interpolated clock drift
%     tmp2 = [0; rtjdx];
%     [~,Vxyzti(1 + [0 rcjdx], 4)] = ...
...
%             (Cclock_sig(cedx + rcidx).^2)' + 1e-22);
    ...
    % add interpolated clock drift - RW 8/22/22 modified
    tmp2 = [rtjdx];
    [~,Vxyzti(1 + [rcjdx], 4)] = ...
            navsu.geo.polyinterp(rtf', Cclock(cedx + rcidx)',cdfit,tmp2, ...
            (Pevent(rpidx +idx) | ...
