function pos = propNavMsg(eph, prn, GPSweek, GPSsec, constellation,dualFreq)
...
pos = struct('x', NaN(tmArrayLen, 1), 'y', NaN(tmArrayLen, 1), 'z', NaN(tmArrayLen, 1), ...
    ...
    'Fit_interval', NaN(tmArrayLen, 1),'TGD',NaN(tmArrayLen,1),'t',NaN(tmArrayLen,1),...
    'suglInfo1',NaN(tmArrayLen,1),'suglInfo2',NaN(tmArrayLen,1));
...
%                 idx = find(eph.PRN == prn(loop) & galEphSource == 1);
            case 'BDS'
                idx = find(eph.PRN == prn(loop));
        end
%     end
...
%     t = mod(t + 302400, 604800) - 302400; % !!
    ...
    if isfield(eph,'suglInfo1') && isfield(eph,'suglInfo2')
       pos.suglInfo1(loop) = eph.suglInfo1(I);
       pos.suglInfo2(loop) = eph.suglInfo2(I);
    end
